...
def boca_aberta(pontos: T_Rosto) -> bool:
    dist_labios = utils.distancia(pontos[13][y], pontos[14][y])
    maior_labio = max(
        utils.distancia(pontos[13][y], pontos[0][y]),
        utils.distancia(pontos[14][y], pontos[17][y]),
    )
    return dist_labios > maior_labio


@registrar_validador(Expressao.BOCA_ABERTA)
def validar_expressao_boca_aberta(rosto: T_Rosto) -> Resultado:
...
def validar_expressao_boca_fechada(rosto: T_Rosto) -> Resultado:
    if boca_aberta(rosto):
...
def validar_expressao_qualquer(rosto: T_Rosto) -> Resultado:
    return Valido()
